...
import {
  ...
  CheckCircle, AlertCircle, Users, Zap,
  ExternalLink, Clock
} from "lucide-react";
...
// ─── Types ────────────────────────────────────────────────────────────────────
interface Email { id: string; from: string; subject: string; body: string; status: string; tag: string; }
interface DaySlot { hour: number; busy: boolean; event?: string | null; status: string; display: string; time: string; }
interface CalEvent { id: string; summary: string; start: string; end: string; link: string; attendees: string[]; }
type Tab = "emails" | "calendar";

// ─── Helpers ──────────────────────────────────────────────────────────────────
function statusMeta(status: string) {
  if (!status) return { dot: "bg-slate-300", pill: "bg-slate-100 text-slate-500" };
  if (status.includes("✅") || status.includes("Scheduled"))  return { dot: "bg-emerald-400", pill: "bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200" };
  if (status.includes("🤖") || status.includes("Booked"))     return { dot: "bg-blue-400",    pill: "bg-blue-50 text-blue-700 ring-1 ring-blue-200" };
  if (status.includes("🔁") || status.includes("Rescheduled"))return { dot: "bg-violet-400",  pill: "bg-violet-50 text-violet-700 ring-1 ring-violet-200" };
  if (status.includes("🟡") || status.includes("Busy"))       return { dot: "bg-amber-400",   pill: "bg-amber-50 text-amber-700 ring-1 ring-amber-200" };
  if (status.includes("❌") || status.includes("No "))        return { dot: "bg-red-400",     pill: "bg-red-50 text-red-700 ring-1 ring-red-200" };
  if (status.includes("❓") || status.includes("Ambiguous"))  return { dot: "bg-orange-400",  pill: "bg-orange-50 text-orange-700 ring-1 ring-orange-200" };
...
const AVATAR_COLORS = [
  ...
  "from-rose-500 to-rose-600", "from-cyan-500 to-cyan-600", "from-indigo-500 to-indigo-600",
];
...
function EmailRow({ mail, index }: { mail: Email; index: number }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.035, duration: 0.28 }}>
      <div onClick={() => setOpen(!open)} className="flex cursor-pointer items-start gap-4 rounded-2xl border border-slate-200/50 bg-white px-5 py-4 transition-all hover:border-slate-300/60 hover:shadow-[0_4px_24px_rgba(15,23,42,0.07)]">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1">
            <span className="text-sm font-semibold text-slate-800 truncate max-w-[260px]">{mail.from}</span>
            {mail.tag?.includes("Meeting") && (
              <span className="rounded-full bg-indigo-100 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-indigo-700 shrink-0">Meeting</span>
            )}
          </div>
          <p className="mt-0.5 text-sm text-slate-600 truncate">{mail.subject || <span className="italic text-slate-400">(no subject)</span>}</p>
          {open && mail.body && (
            <motion.p initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="mt-2 text-xs leading-relaxed text-slate-500 border-t border-slate-100 pt-2 overflow-hidden">
              {mail.body}
            </motion.p>
          )}
        </div>
        <div className="flex shrink-0 flex-col items-end gap-2">
          ...
          {mail.status && (
            <span className={`rounded-full px-2.5 py-1 text-[11px] font-medium ${pill}`}>{mail.status}</span>
          )}
        </div>
      </div>
    </motion.div>
...
function CreatedEventsPanel() {
  ...
  const load = () => {
    ...
    fetch(`${BACKEND}/check_calendar`)
      .then(r => r.json())
      .then(d => {
        if (d.error && d.error === "not_logged_in") setError("Not logged in to Google.");
        else setEvents(d.events || []);
        setLoading(false);
      })
      .catch(e => { setError("Could not reach backend."); setLoading(false); });
  };
  ...
  return (
    <div className="rounded-2xl border border-slate-200/50 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-semibold text-slate-800">Upcoming Calendar Events</h3>
          <p className="text-xs text-slate-400 mt-0.5">Events created by MailFlow AI</p>
        </div>
        <button onClick={load} className="flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50 transition-colors">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-10 text-center gap-2">
          ...
          <p className="text-xs text-slate-400">Events will appear here once AI schedules meetings from your inbox</p>
        </div>
      ) : (
        <div className="space-y-2">
          {events.map(ev => (
            <div key={ev.id} className="flex items-start gap-3 rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-3">
              <CheckCircle className="h-4 w-4 text-emerald-500 mt-0.5 shrink-0" />
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <span className="text-sm font-semibold text-slate-800">{ev.summary}</span>
                  {ev.link && (
                ...
                <p className="text-xs text-slate-500 mt-0.5">{fmtTime(ev.start)} → {fmtTime(ev.end)}</p>
                {ev.attendees.length > 0 && (
                  <p className="text-xs text-slate-400 mt-0.5">With: {ev.attendees.join(", ")}</p>
                )}
              </div>
            </div>
          ))}
...
function CalendarView() {
  ...
  useEffect(() => {
    ...
    fetch(`${BACKEND}/day_slots/${dateStr}`)
      .then(r => r.json()).then(d => { setSlots(Array.isArray(d) ? d : []); setSL(false); })
      .catch(() => { setSlots([]); setSL(false); });
  }, [dateStr]);
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="rounded-2xl border border-slate-200/50 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex gap-0.5">
              <button onClick={() => setCur(new Date(yr, mo - 1, cur.getDate()))} className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-700 transition-colors"><ChevronLeft className="h-4 w-4" /></button>
              <button onClick={() => setCur(new Date(yr, mo + 1, cur.getDate()))} className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-700 transition-colors"><ChevronRight className="h-4 w-4" /></button>
            </div>
          </div>
              ...
              return (
                <button key={day} onClick={() => setCur(new Date(yr, mo, day))}
                  className={`relative mx-auto flex h-8 w-8 items-center justify-center rounded-full text-sm font-medium transition-all
                    ${sel ? "bg-slate-900 text-white shadow" : tod ? "bg-blue-100 text-blue-700" : "text-slate-600 hover:bg-slate-100"}`}>
                  {day}
                  {busy && !sel && <span className="absolute bottom-0.5 left-1/2 -translate-x-1/2 h-1 w-1 rounded-full bg-blue-500" />}
                </button>
              );
            })}
          </div>
            ...
            <div className="space-y-1.5 max-h-72 overflow-y-auto pr-1">
              {slots.map(slot => (
                <div key={slot.hour} className={`flex items-center gap-3 rounded-xl px-4 py-2.5 ${slot.busy ? "bg-rose-50 border border-rose-100" : "bg-emerald-50 border border-emerald-100"}`}>
                  <span className="w-14 shrink-0 font-mono text-xs text-slate-500">{String(slot.hour).padStart(2,"0")}:00</span>
      ...
      {/* Created events verification panel */}
      <CreatedEventsPanel />
...
export default function AppPage() {
  ...
  const load = (refresh = false) => {
    if (refresh) setSpin(true); else setLoad(true);
    fetch(`${BACKEND}/emails`)
      .then(r => r.json())
      .then(d => { setEmails(Array.isArray(d.emails) ? d.emails : []); setToday(d.today || ""); setLoad(false); setSpin(false); })
      .catch(() => { setLoad(false); setSpin(false); });
  };
    ...
    <div className="flex h-screen overflow-hidden bg-[#f5f5f7]">
      ...
      <aside className="flex w-[68px] shrink-0 flex-col items-center gap-3 border-r border-slate-200/70 bg-white/80 py-5 backdrop-blur-xl">
        ...
        <div className="mt-4 flex flex-1 flex-col items-center gap-1.5">
          <button onClick={() => setTab("emails")} title="Inbox"
            className={`flex h-10 w-10 items-center justify-center rounded-xl transition-all ${tab==="emails" ? "bg-slate-900 text-white shadow" : "text-slate-400 hover:bg-slate-100 hover:text-slate-700"}`}>
            <Inbox className="h-5 w-5" />
          </button>
          <button onClick={() => setTab("calendar")} title="Calendar"
            className={`flex h-10 w-10 items-center justify-center rounded-xl transition-all ${tab==="calendar" ? "bg-slate-900 text-white shadow" : "text-slate-400 hover:bg-slate-100 hover:text-slate-700"}`}>
            <Calendar className="h-5 w-5" />
          </button>
        </div>
        <button onClick={() => { window.location.href = `${BACKEND}/logout`; }} title="Logout"
      ...
      <main className="flex flex-1 flex-col overflow-hidden">
        <header className="flex items-center justify-between border-b border-slate-200/60 bg-white/70 px-8 py-4 backdrop-blur-xl">
          <div>
            <h1 className="text-xl font-semibold tracking-tight text-slate-900" style={{ fontFamily: "Playfair Display, Georgia, serif" }}>
              ...
              <motion.div key="emails" initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.18 }}>
                ...
                {loading ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-32 text-center gap-4">
                    ...
                    <button onClick={() => { window.location.href = `${BACKEND}/login`; }}
                  ...
                  <div className="space-y-2">
                    {emails.map((mail, i) => <EmailRow key={mail.id || i} mail={mail} index={i} />)}
                  </div>
                )}
              </motion.div>
            ) : (
